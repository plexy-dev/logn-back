...
func (aq *AreaQuery) QueryVacancies() *VacancyQuery {
	query := (&VacancyClient{config: aq.config}).Query()
...
func (aq *AreaQuery) WithVacancies(opts ...func(*VacancyQuery)) *AreaQuery {
	query := (&VacancyClient{config: aq.config}).Query()
...
func (aq *AreaQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Area, error) {
	...
	if query := aq.withVacancies; query != nil {
...
func (aq *AreaQuery) loadVacancies(ctx context.Context, query *VacancyQuery, nodes []*Area, init func(*Area), assign func(*Area, *Vacancy)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(area.VacanciesTable)
		s.Join(joinT).On(s.C(vacancy.FieldID), joinT.C(area.VacanciesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(area.VacanciesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(area.VacanciesPrimaryKey[0]))
