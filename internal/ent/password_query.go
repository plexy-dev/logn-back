...
func (pq *PasswordQuery) QueryUsers() *UserQuery {
...
func (pq *PasswordQuery) WithUsers(opts ...func(*UserQuery)) *PasswordQuery {
	query := (&UserClient{config: pq.config}).Query()
...
func (pq *PasswordQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Password, error) {
	...
	if query := pq.withUsers; query != nil {
		if err := pq.loadUsers(ctx, query, nodes,
			func(n *Password) { n.Edges.Users = []*User{} },
			func(n *Password, e *User) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PasswordQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Password, init func(*Password), assign func(*Password, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Password)
	nids := make(map[int]map[*Password]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(password.UsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(password.UsersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(password.UsersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(password.UsersPrimaryKey[0]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Password]struct{}{byID[outValue]: {}}
