...
type User struct {
	...
	// BirthMonth holds the value of the "birth_month" field.
...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case user.FieldID, user.FieldBirthDay, user.FieldBirthMonth, user.FieldBirthYear:
	...
	for i := range columns {
		...
		case user.FieldBirthDay:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				u.BirthDay = int(value.Int64)
			}
		case user.FieldBirthMonth:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field birth_month", values[i])
			} else if value.Valid {
				u.BirthMonth = int(value.Int64)
			}
...
func (u *User) QueryCommunities() *CommunityQuery {
	return NewUserClient(u.config).QueryCommunities(u)
}
...
func (u *User) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("birth_month=")
	builder.WriteString(fmt.Sprintf("%v", u.BirthMonth))
	builder.WriteString(", ")
