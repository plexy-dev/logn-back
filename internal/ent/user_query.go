...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withCommunities; query != nil {
		if err := uq.loadCommunities(ctx, query, nodes,
			func(n *User) { n.Edges.Communities = []*Community{} },
			func(n *User, e *Community) { n.Edges.Communities = append(n.Edges.Communities, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadPasswords(ctx context.Context, query *PasswordQuery, nodes []*User, init func(*User), assign func(*User, *Password)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.PasswordsTable)
		s.Join(joinT).On(s.C(password.FieldID), joinT.C(user.PasswordsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.PasswordsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.PasswordsPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*Password](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "passwords" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (uq *UserQuery) loadCommunities(ctx context.Context, query *CommunityQuery, nodes []*User, init func(*User), assign func(*User, *Community)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*User)
	nids := make(map[int]map[*User]struct{})
	for i, node := range nodes {
...
func (uq *UserQuery) loadCompanies(ctx context.Context, query *CompanyQuery, nodes []*User, init func(*User), assign func(*User, *Company)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(company.FieldID), joinT.C(user.CompaniesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.CompaniesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.CompaniesPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
