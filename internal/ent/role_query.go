...
func (rq *RoleQuery) QueryCompanies() *CompanyQuery {
...
func (rq *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withCompanies: rq.withCompanies.Clone(),
...
func (rq *RoleQuery) WithCompanies(opts ...func(*CompanyQuery)) *RoleQuery {
	query := (&CompanyClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withCompanies = query
	return rq
}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withCompanies; query != nil {
		if err := rq.loadCompanies(ctx, query, nodes,
			func(n *Role) { n.Edges.Companies = []*Company{} },
			func(n *Role, e *Company) { n.Edges.Companies = append(n.Edges.Companies, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoleQuery) loadCompanies(ctx context.Context, query *CompanyQuery, nodes []*Role, init func(*Role), assign func(*Role, *Company)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Role)
	nids := make(map[int]map[*Role]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(role.CompaniesTable)
		s.Join(joinT).On(s.C(company.FieldID), joinT.C(role.CompaniesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(role.CompaniesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.CompaniesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
