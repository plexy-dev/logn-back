...
func (rc *RoleCreate) AddCompanyIDs(ids ...int) *RoleCreate {
	rc.mutation.AddCompanyIDs(ids...)
	return rc
}

// AddCompanies adds the "companies" edges to the Company entity.
func (rc *RoleCreate) AddCompanies(c ...*Company) *RoleCreate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return rc.AddCompanyIDs(ids...)
}
...
func (rc *RoleCreate) createSpec() (*Role, *sqlgraph.CreateSpec) {
	...
	if nodes := rc.mutation.CompaniesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
