...
func (lq *LocationQuery) QueryVacancies() *VacancyQuery {
	query := (&VacancyClient{config: lq.config}).Query()
...
func (lq *LocationQuery) Clone() *LocationQuery {
	...
	return &LocationQuery{
		...
		withVacancies: lq.withVacancies.Clone(),
...
func (lq *LocationQuery) WithVacancies(opts ...func(*VacancyQuery)) *LocationQuery {
	query := (&VacancyClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	lq.withVacancies = query
	return lq
}
...
func (lq *LocationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Location, error) {
	...
	if query := lq.withVacancies; query != nil {
		if err := lq.loadVacancies(ctx, query, nodes,
			func(n *Location) { n.Edges.Vacancies = []*Vacancy{} },
			func(n *Location, e *Vacancy) { n.Edges.Vacancies = append(n.Edges.Vacancies, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LocationQuery) loadVacancies(ctx context.Context, query *VacancyQuery, nodes []*Location, init func(*Location), assign func(*Location, *Vacancy)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(location.VacanciesTable)
		s.Join(joinT).On(s.C(vacancy.FieldID), joinT.C(location.VacanciesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(location.VacanciesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(location.VacanciesPrimaryKey[0]))
